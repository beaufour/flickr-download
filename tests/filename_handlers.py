...
class TestFilenameHandlers(unittest.TestCase):
    ...
    def test_title(self):
        ...
        self.assertEqual(fn(self._pset, self._photo, self._suffix), "Some Photo")

    def test_title_empty_title(self):
        photo = AttrDict({"title": "", "id": 192})
        fn = get_filename_handler("title")
        self.assertEqual(fn(self._pset, photo, self._suffix), "192")

    def test_title_and_id(self):
        fn = get_filename_handler("title_and_id")
        self.assertEqual(fn(self._pset, self._photo, self._suffix), "Some Photo-123")
    ...
    def test_title_increment(self):
        ...
        self.assertEqual(fn(self._pset, self._photo, self._suffix), "Some Photo")
        self.assertEqual(fn(self._pset, self._photo, self._suffix), "Some Photo(1)")

        # Ensure no increment on different title
        photo2 = AttrDict({"title": "Some Other Photo", "id": 124})
        self.assertEqual(fn(self._pset, photo2, self._suffix), "Some Other Photo")
    ...
    def test_valid_path(self):
        ...
        fn = get_filename_handler("title")
        self.assertEqual(fn(self._pset, photo, self._suffix), "file_path")

        fn = get_filename_handler("title_increment")
        self.assertEqual(fn(self._pset, photo, self._suffix), "file_path")

        fn = get_filename_handler("title_and_id")
        self.assertEqual(fn(self._pset, photo, self._suffix), "file_path-199")
